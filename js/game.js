...
function getResetGain(layer, useType = null) {
	...
	if (type=="static") {
		...
		return new Decimal(Number(gain.floor().sub(player[layer].points).add(1).max(1)).toString(player.b.points));
	} else if (type=="normal"){
		...
		gain = gain.times(tmp[layer].directMult)
		return new Decimal(Number(gain.floor().max(0)).toString(player.b.points));
	} else if (type=="custom"){
		return layers[layer].getResetGain()
	} else {
		return decimalZero
...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
		...
		return new Decimal(Number(cost).toString(player.b.points));
	} else if (type=="normal"){
		...
		return new Decimal(Number(next).toString(player.b.points));
	} else if (type=="custom"){
		return layers[layer].getNextAt(canMax)
	} else {
		return decimalZero
